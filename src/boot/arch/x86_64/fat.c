...
fat_load_kernel(struct blkdev *blkdev, u64 addr)
{
    ...
    blkdev->read(blkdev, 0, 8, buf);

    u32 lba = *(u32 *)(buf + 0x1be + 0x8);
    u32 nsec = *(u32 *)(buf + 0x1be + 0xc);


    bpb = (struct fat_bpb_fat12_16 *)bmalloc(4096);
    blkdev->read(blkdev, lba, 8, bpb);
    ...
    root_dir_sectors = (32 * bpb->common.bpb_root_ent_cnt
                        + bpb->common.bpb_bytes_per_sec - 1)
    ...
    /* For cluster */
    cluster = bmalloc(bpb->common.bpb_fat_sz16 * bpb->common.bpb_num_fats
                      * bpb->common.bpb_bytes_per_sec);
    blkdev->read(blkdev, lba +
                 (start_sec * bpb->common.bpb_bytes_per_sec) / 512,
                 (bpb->common.bpb_fat_sz16 * bpb->common.bpb_num_fats
                     * bpb->common.bpb_bytes_per_sec) / 512,
                 cluster);

    /* Allocate memory */
    buf = bmalloc(root_dir_sectors * bpb->common.bpb_bytes_per_sec);
    /* Root directory */
    blkdev->read(blkdev, lba +
                 (root_dir_start_sec * bpb->common.bpb_bytes_per_sec) / 512,
                 (root_dir_sectors * bpb->common.bpb_bytes_per_sec) / 512,
                 buf);

    int i;
    for ( i = 0; i < root_dir_sectors * bpb->common.bpb_bytes_per_sec / 32; i++ ) {
